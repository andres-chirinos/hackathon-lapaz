...
def run_agent(nl_query: str, db_schema: str, knowledge_collection, conn, api_key: str, user_context: str = ""):
    ...
    with st.status("⚙️ Fase 2: Desarrollo de Consultas", expanded=True) as s2:
        ...
        Manejo de Gran Volumen y Agrupación Espacial: NUNCA utilices `AVG(latitud)` o `AVG(longitud)` para agrupar puntos en un solo centroide promedio, ya que esto distorsiona la visualización real. Si los datos son demasiados, tienes dos opciones: 1) Retornar todos los puntos crudos (hasta un límite razonable como LIMIT 2000). 2) Si debes agrupar por una dimensión espacial (municipio, departamento o macroregión), SIEMPRE debes hacer un JOIN obligatorio con las tablas de la capa `frontlines` (ej. `frontlines_municipios`) y retornar la geometría exacta (`ST_AsText(geom) AS geometry`) como la representación del grupo, sin promediar las coordenadas crudas.
        Series de Tiempo Obligatorias: Si el usuario pregunta por la evolución de una métrica en un lapso de tiempo (ej. "en el último mes", "tendencia", "histórico"), es OBLIGATORIO que agrupes (GROUP BY) por una cubeta de tiempo usando `DATE_TRUNC('day', fecha)` o `DATE_TRUNC('month', fecha)`. NUNCA devuelvas una sola fila con el promedio global de todo el mes si el usuario espera ver un gráfico de evolución. Necesitamos múltiples filas (una por día/mes) para construir el gráfico de líneas.
